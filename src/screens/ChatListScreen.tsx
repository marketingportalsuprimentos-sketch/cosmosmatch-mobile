...
export const ChatListScreen = ({ navigation }: any) => {
  ...
  const { data: conversations, isLoading: loadingChat } = useGetConversations();
  const { data: likes, isLoading: loadingLikes } = useGetLikesReceived();
      ...
      <View style={styles.card}>
        ...
        >
            <View style={styles.avatarContainer}>
                <Image 
                    source={{ uri: otherParticipant.user.profile?.imageUrl || 'https://via.placeholder.com/150' }} 
                    style={styles.avatar} 
                />
  ...
  const renderLikeItem = ({ item }: any) => (
    ...
    >
        <Image 
            ...
            style={[styles.avatar, { borderColor: '#A78BFA', borderWidth: 2 }]} 
        />
        <View style={styles.textContainer}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        ...
        >
            ...
            {(unreadLikesData?.count || 0) > 0 && (
                <View style={styles.counterBadge}>
                    <Text style={styles.counterText}>{unreadLikesData?.count}</Text>
                </View>
            )}
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.content}>
        {activeTab === 'messages' ? (
            loadingChat ? (
                <ActivityIndicator color="#A78BFA" style={{marginTop: 50}} />
            ) : (
                <FlatList
                    data={conversations || []}
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            ...
                            <Text style={styles.emptyText}>Suas conexões aparecerão aqui.</Text>
                        </View>
                    }
                />
            )
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            ...
                            <Text style={styles.emptyTitle}>Sem likes ainda</Text>
                            <Text style={styles.emptyText}>Capriche no perfil para atrair conexões!</Text>
                        </View>
                    }
                />
            )
...
const styles = StyleSheet.create({
  ...
  tabsContainer: {
    flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#374151',
    marginBottom: 10,
  },
  tab: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 15, gap: 8, borderBottomWidth: 2, borderBottomColor: 'transparent'
  },
  ...
  card: {
    ...
    backgroundColor: '#1F2937', borderRadius: 12, padding: 12, marginBottom: 10,
