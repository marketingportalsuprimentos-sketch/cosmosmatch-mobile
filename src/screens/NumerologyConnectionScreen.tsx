...
export const NumerologyConnectionScreen = () => {
  ...
  if (error || !data) return <View style={styles.center}><Text style={styles.errorText}>{t('error_loading_numerology')}</Text></View>;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('numerology_connection_title')}</Text>
        <View style={{width: 24}} />
      </View>

      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.subtitle}>
            {t('compatibility_between')} <Text style={{color:'#818CF8'}}>{data.nameA}</Text> {t('and')} <Text style={{color:'#818CF8'}}>{data.nameB}</Text>
        </Text>
